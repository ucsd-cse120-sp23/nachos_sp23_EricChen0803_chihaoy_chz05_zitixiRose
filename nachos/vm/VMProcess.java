...
public class VMProcess extends UserProcess {
	...
	private void swap(int ppn){
		if (VMkernel.IPT[ppn].entry.dirty == 1){
			int spn = 0;
			if (freeswappagelist.size() != 0){//if there is free swap page number left, then use this
				spn = freeswappagelist.removeLast();
			}
			...
			VMkernel.swapFile.write(spn * Processor.pageSize,Machine.processor().getMemory(),Processor.makeAddress(VMkernel.IPT[ppn]),0,Processot.pageSize);//write to swap file
