...
public class VMProcess extends UserProcess {
	...
	public void handlePageFault(){
		...
		for (int s = 0; s < coff.getNumSections(); s++) {
			...
			int coffVpn = section.getFirstVPN();
			int coffVpnEnd = coffVpn+section.getLength();
			if((badVpn >= coffVpn) && (badVpn <= coffVpnEnd)){
				flag = true;
				break;
			}
		}
		...
		int ppn1 = 0;
		if(flag==true){
			CoffSection section = coff.getSection(mark);
			boolean readOnly = section.isReadOnly();
			for (int i = 0; i < section.getLength(); i++) {
				int vpn = section.getFirstVPN() + i;
				int ppn = freeList.remove();
				ppn1 = ppn;
				pageTable[vpn] = new TranslationEntry(vpn, ppn1, true, readOnly, false, false);
				section.loadPage(i, pageTable[vpn].ppn);
			}
		}
		else{
			/*Use System.arraycopy to zero_filled */
			//byte[] data = new byte[Processor.pageSize];
			//System.arraycopy(data,0,processor.getMemory(),processor.makeAddress(0,0),Processor.pageSize);
		}
		return;
	}
